  ...
  switch (action.type) {
    ...
    case "UpdateArmiesInTable":
      ...
      return updateTableForValue(
        state,
        action.newValue,
        isNaN(parseInt(action.newValue)) ? Intent.DANGER : Intent.NONE,
        action.rowIndex,
        3
      );
  }
...
const TerritorySelect = (props: ITerritorySelectProps) => {
  ...
  const cellRenderer = (rowIndex: number, columnIndex: number) => {
    if (columnIndex === 2) {
      return (
        <EditableCell
          ...
          onCancel={changePlayer}
          onConfirm={changePlayer}
    ...
    if (columnIndex === 3) {
      return (
        <EditableCell
          value={state.dataForTable[rowIndex][columnIndex]}
          onChange={changeArmies}
          onCancel={changeArmies}
          onConfirm={changeArmies}
          intent={state.cellIntents[rowIndex][columnIndex]}
          rowIndex={rowIndex}
          columnIndex={columnIndex}
        ></EditableCell>
  ...
  const player1NameUpdated: ChangeEventHandler<HTMLInputElement> = function (
    onChangeArgs: ChangeEvent<HTMLInputElement>
  ) {
    const newValue = onChangeArgs.target.value;
    dispatch({ type: "UpdatePlayer1Name", newValue: newValue });
  };

  const player2NameUpdated: ChangeEventHandler<HTMLInputElement> = function (
    onChangeArgs: ChangeEvent<HTMLInputElement>
  ) {
    const newValue = onChangeArgs.target.value;
  ...
  return (
    <>
      ...
      <Card>
        ...
        <Table2 numRows={state.dataForTable.length}>{columns}</Table2>
        <Button onClick={submitData} disabled={state.canSubmitData === false}>
          Start Game
        </Button>
