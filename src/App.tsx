...
function App() {
  ...
  let switchPageToDisplay =  (state : IAppState) => {
    switch (state.currentPage) {
      case 'Loading':
        return (<><br /><br /><Spinner size={300} intent={Intent.WARNING}></Spinner></>)
    ...
    <div className="App">
      <main className="Main p-5">
        <div className="container-fluid">
          <div className="row">
            <GameContext.Provider value={{ ...state.currentContext, currentPositions: state.startingPositions, currentPlayers: state.players }}>
...
const appStateReducer = (state: IAppState, action: IAppStateAction) => {
  if (action.type === "PositionsSelected"
    ...
    const newState: IAppState = { currentPage: 'WorldMap', startingPositions: action.startingPositions, players: action.players, currentContext: state.currentContext };
    return newState;
  }
  if (action.type === "MapDataLoaded"
    && action.newContext !== undefined) {
    const newState: IAppState = { currentPage: 'TerritorySelect', startingPositions: state.startingPositions, players: state.players, currentContext: action.newContext };
    return newState;
  }
  return state;
};
